...
// Lazy Agent Memory pattern-detection — much longer default interval
// than the GSC fetch above (memory_thresholds_json.detection_interval_days,
// default 7 days): "is this pattern still consistent" changes slowly, so
// re-running the full-window analysis every page load would be wasted
// work. No-op if GSC isn't connected yet. See docs/GROWTH_AGENT_MEMORY_PLAN.md.
...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
    ...
    // call, see cms_growth_agent_detect_memory_patterns()).
...
<section class="admin-stack">
    ...
    <div class="panel">
        <div class="panel__head">
            ...
            <span class="panel__meta"><?= count($memoryPending) ?> pending, <?= count($memoryActive) ?> active</span>
        </div>
        ...
        <form method="post" action="<?= cms_esc($selfUrl) ?>" style="padding:0 20px 16px;">
            <?= cms_csrf_field() ?>
            <input type="hidden" name="action" value="analyze_memory_patterns">
            <button type="submit" class="admin-btn admin-btn--secondary">Analisis Pola</button>
        </form>

        <?php if ($memoryPending !== []) : ?>
            <div style="padding:0 20px 8px;">
                <h4 style="margin:0;font-size:14px;">Pending Review</h4>
            </div>
            <div class="table-wrap">
                <table class="admin-table">
                    <thead>
        ...
        <div style="padding:16px 20px 8px;">
            <h4 style="margin:0;font-size:14px;">Active</h4>
        </div>
        <div class="table-wrap">
            <table class="admin-table">
                <thead>
